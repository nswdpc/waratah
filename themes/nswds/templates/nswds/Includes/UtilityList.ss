<div class="nsw-utility-list js-utility-list<% if not $UtilityList_Vertical %> nsw-utility-list--horizontal<% end_if %>">

  <ul>

    <% if $UtilityList_Features.copylink %>
    <li class="nsw-utility-list__item js-copy-clipboard">
      <% include nswds/Icon Icon_Icon='link' %>
      <span><%t nswds.FEATURE_COPY_LINK 'Copy link' %></span>
    </li>
    <% end_if %>

    <% if $UtilityList_Features.print %>
    <li class="nsw-utility-list__item js-print-page">
      <% include nswds/Icon Icon_Icon='print' %>
      <span><%t nswds.FEATURE_PRINT 'Print this page' %></span>
    </li>
    <% end_if %>

    <% if $UtilityList_Features.download %>
    <li class="nsw-utility-list__item js-download-page"<% if $UtilityList_ContentClass %> data-pdf-content="{$UtilityList_ContentClass.HTMLATT}"<% end_if %>>
     <% include nswds/Icon Icon_Icon='download' %>
      <span><%t nswds.FEATURE_DOWNLOAD 'Download as PDF' %></span>
    </li>
    <% end_if %>
    ...
    <li class="nsw-utility-list__item js-share" aria-controls="<% if $UtilityList_ShareID %>{$UtilityList_ShareID}<% else %>utility-list<% end_if %>-social-share" data-title="<%t nswds.SOCIAL_SHARING 'Social Sharing' %>">
      ...
      <div id="<% if $UtilityList_ShareID %>{$UtilityList_ShareID}<% else %>utility-list<% end_if %>-social-share" class="nsw-toggletip__element nsw-toggletip__element--light">
        ...
        <% if $UtilityList_Networks.linkedin %>
            <a class="nsw-icon-button" data-social="linkedin" data-url="<% if $UtilityList_CurrentPageURL %>{$UtilityList_CurrentPageURL.HTMLATT}<% end_if %>" href="https://www.linkedin.com/shareArticle">
        ...
        <% if $UtilityList_Networks.twitter %>
            <a class="nsw-icon-button" data-social="twitter" data-text="{$UtilityList_Prompt.HTMLATT}" data-hashtags="{$UtilityList_TwitterTags.HTMLATT}" data-username="" href="https://twitter.com/intent/tweet" aria-label="<%t nswds.SHARE_ON_X_TWITTER 'Share on X (Twitter)' %>">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path d="M389.2 48h70.6L305.6 224.2 487 464H345L233.7 318.6 106.5 464H35.8L200.7 275.5 26.8 48H172.4L272.9 180.9 389.2 48zM364.4 421.8h39.1L151.1 88h-42L364.4 421.8z"/></svg>
            </a>
        <% end_if %>

        <% if $UtilityList_Networks.email %>
            <a class="nsw-icon-button" data-social="mail" data-subject="{$UtilityList_Prompt.HTMLATT}" data-body="{$UtilityList_Body.HTMLATT}" href="mailto:" aria-label="<%t nswds.SHARE_VIA_EMAIL 'Share via Email' %>">
              <% include nswds/Icon Icon_Icon='email' %>
            </a>
        <% end_if %>
